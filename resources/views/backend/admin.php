...
function adminlog( ){   
    ...
    if( $postlog || $getlog ){
        ...
        $ip = get_client_ip();
        $msql->query( "INSERT INTO cpp_base_adminlog SET coltype='{$coltype}',sysuser='{$sysuser}',postlog='{$postlog}',getlog='{$getlog}',reqlog='{$reqlog}',logtime='{$logtime}',time='{$time}',ip='{$ip}' " );
    }
}
...
// </html>';
                ...
                $msql->query( "select * from cpp_base_admin where user='".$_COOKIE['SYSUSER']."'" );
                if ( $msql->next_record( ) )
                {
                                $psd = $msql->f( "password" );
                                ...
                                if ( $au != "0" )
                                {
                                                $msql->query( "select * from cpp_base_adminrights where user='".$_COOKIE['SYSUSER']."' and auth='{$au}'" );
                                                if ( $msql->next_record( ) )
                                                {
                                                }
                                                else
...
function showauth( $au, $style="1" )
{
                ...
                if ( !is_array($au) )
                {
                    $tsql->query( "select * from cpp_base_adminrights where user='".$_COOKIE['SYSUSER']."' and auth='{$au}'" );
                    if ( $tsql->next_record( ) )
                    {
                        return "";
                    }
                    ...
                    $aus = implode(",",$au);
                    $tsql->query( "select COUNT(*) from cpp_base_adminrights where user='".$_COOKIE['SYSUSER']."' and auth IN (".$aus.")" );
                    if ( $tsql->next_record( ) )
                    {
                        if ( $tsql->f('COUNT(*)') > 0 )
                        {
...
{
                global $msql;
                $msql->query( "select * from cpp_base_config" );
...
{
                global $msql;
                $msql->query( "select count(".$id.") from cpp".$tbl." where {$scl}" );
                if ( $msql->next_record( ) )
                {
...
{
                global $tsql;
                $tsql->query( "select sname from cpp_base_coltype where coltype='{$coltype}'" );
                if ( $tsql->next_record( ) )
                {
                                $sname = $tsql->f( "sname" );
                }
...
{
                global $fsql;
                $fsql->query( "select colname from cpp_base_coltype where coltype='{$coltype}'" );
                if ( $fsql->next_record( ) )
                {
...
function tblinsert( $tbl, $vars )
{
                ...
                $msql->query( "insert into cpp".$tbl." set {$scl}" );
}
